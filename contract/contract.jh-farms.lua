...
CONTRACT_OTC_TOKEN = "contract.jh-token-otc"
CONTRACT_OTC_FARMS = "contract.jh-farms-otc"
...
local function _ContractOTCToken()
    COTCToken = import_contract(CONTRACT_OTC_TOKEN)
end

local function _ContractOTCFarms()
    COTCFarms = import_contract(CONTRACT_OTC_FARMS)
end
...
local function _get_nft_contract_info(nft_id)
    ...
    assert(nft.world_view == G_CONFIG.FARMS_WORLD_VIEW,
           "#世界观不正确！#")
    for _, contract in pairs(nft.describe_with_contract) do
...
function create()
    ...
    -- 创建nft共享数据
    local base_info = {name = attrs.name .. "的农田"}
    local nft_id = chainhelper:create_nft_asset(contract_base_info.owner,
                                                G_CONFIG.FARMS_WORLD_VIEW,
                                                cjson.encode(base_info), true,
                                                true)
    chainhelper:change_nht_active_by_owner(contract_base_info.caller, nft_id,
                                           false)
    -- 10块农田写入信息
    local nft_contract_info = {}
    for i = 0, 9 do nft_contract_info["land_" .. i] = {} end
    nft_contract_info.owner = contract_base_info.caller
    -- 记录农田信息
    private_data.farm = {nft_id = nft_id}
    chainhelper:nht_describe_change(nft_id, NFT_CONTRACT_INFO,
                                    cjson.encode(nft_contract_info), false)
    chainhelper:change_nht_active_by_owner(contract_base_info.owner, nft_id,
                                           false)
...
function BuySeed(args)
    ...
    local args = {{2, {v = seed_id}}, {1, {v = seed_num}}}
...
function BuyLand(args)
    ...
    assert(land.star < package.goods[shovel_id].base_info.star,
           "#请使用" .. land.star .. "星以上的铁锹升级土地!#")
    land.star = land.star + 1
    -- 消耗一把铁锹
    CPlayerPackage.spent_item(shovel_id, 1)
    chainhelper:log(attrs.name .. land_key .. "升级到了" .. land.star ..
                        "级")
    -- 记录到nft以备共享数据
...
function Plant(args)
    ...
    if seed_id == "g20010102" then 
        local _otc_seed = chainhelper:get_contract_public_data(CONTRACT_OTC_FARMS)
        assert((_otc_seed.reap_num == nil and _otc_seed.total_seed >= 1) or
                   (_otc_seed.total_seed >= (_otc_seed.reap_num + 1)),"免费种子的种植额度已满，请稍后再来种植！")
    end
    -- 消耗一颗种子
...
function Reap(args)
    ...
    if now_time >= timestamp + plant_time then time_long = plant_time end
    -- 保存数据
    ...
    if land.steal_info then
        for _, value in pairs(land.steal_info) do
            if value[1] then
                steal_success_count = steal_success_count + 1
            end
        end
    end
    local total = plant.uint * time_long * efficiency *
                      (1 - 0.1 * steal_success_count) / 10
    total = math.floor(total)
    chainhelper:log('收获' .. (total / 100000) .. token_id)
    -- 解锁并转发到账户
    CToken.TransferOut(token_id, total)
    if plant.gid == "g20010102" then
        -- 场外提供的种子，没有成熟，不能收获
        assert(time_long == plant_time,
               "#场外提供的种子，未完全成熟不能收获！#")
        local _otc_seed = chainhelper:get_contract_public_data(CONTRACT_OTC_FARMS)
        assert((_otc_seed.reap_num == nil and _otc_seed.total_seed >= 1) or
                (_otc_seed.total_seed >= (_otc_seed.reap_num + 1)),"免费种子的收割额度已满，请稍后再来收割！")
        _ContractOTCToken()
        _ContractOTCFarms()
        -- 95% 转入OTC账户
        COTCToken.TransferIn(token_id, math.floor(total * 0.95))
        -- 种子库 -1
        COTCFarms.Reap()
    end
    -- 记录到nft以备共享数据
...
function Steal(args)
    ...
    for i = 0, 9 do
        if _self_nft_land["land_" .. i].status ~= nil then
            ...
            if _self_land_num >= 2 then break end
        end
    end
    assert(_self_land_num >= 2,
           "#你自己的土地数太少，不能偷窃！#");
    local attrs = private_data.attrs
    ...
    if dog_info == nil then
        dog_info = {growt = 0, timestamp = 0, kill_timestamp = 0}
    end
    local now_time = chainhelper:time()
    -- 狗防御两小时,被揍5钟内可偷
    assert((now_time < (dog_info.kill_timestamp + 300)) or
               (now_time > (dog_info.timestamp + (240 * 60))),
           "#狗很凶，没法偷窃！#")
    local land = nft_land[land_key]
    ...
    assert(land.steal_info[contract_base_info.caller] == nil,
           "#这块地你已经偷过了，给主人留点吧！#")
    assert(land.steal_count < MAX_STEAL_COUNT,
           "#这块地已经损失了操过一半了，给主人留点吧！#")
    local plant = land.plant
    local timestamp = plant.timestamp
    local token_id = plant.token_id
    local plant_time = plant.time
    ...
    chainhelper:transfer_from_caller(contract_base_info.owner, forfeit, "COCOS",
                                     true)
    ...
    if random_number > ((land.star - 1) * 200 + 5000 - luck_num + dog_num) then
        chainhelper:log(contract_base_info.caller .. '偷取' ..
                            (total / 100000) .. token_id)
        -- 解锁并转发到账户
        CToken.TransferOut(token_id, total)
        -- 返还罚金
        chainhelper:transfer_from_owner(contract_base_info.caller, forfeit,
                                        "COCOS", true)
    else
        status = false
        -- 把罚金95%送给农田主人
        ...
        -- 返还罚金
        chainhelper:transfer_from_owner(nft_land.owner, player_forfeit, "COCOS",
                                        true)
        -- log
        chainhelper:log(contract_base_info.caller .. '偷窃被抓,罚款' ..
                            (forfeit / 100000) .. "COCOS")
...
function KeepADog(args)
    ...
    if dog_info == nil then
        ...
        -- 冷却4小时
        assert((dog_info.timestamp + 240 * 60) < now_time,
               "#这条狗已经吃饱了，还没消化呢！#")
...
function KillADog(args)
    ...
    -- 30分钟可打一次
    assert((chainhelper:time() - dog_info.kill_timestamp) > (30 * 60),
           "#这条狗刚被打，求放过！#")
    -- 取决于打狗人的根骨，造成伤害
    dog_info.growt = dog_info.growt -
                         math.floor(attrs.innate_attr.root_bone / 10)
...
-- 领取免费种子
-- 每次领取土地数对应的颗数，每24小时领取一次
-- 领取要求：土地开垦5块及以上的勤劳少侠
function ReceiveSeed()
    -- 免费种子ID
    local seed_id = "g20010102"
    local farm = private_data.farm
    local _self_nft_id = farm.nft_id
    ...
    assert(_self_land_num >= 5,
           "#你自己的土地数太少，不够勤劳，奸商不信任你！#");
    local _otc_seed = chainhelper:get_contract_public_data(CONTRACT_OTC_FARMS)
    -- 有这么多种子才能领！
    assert((_otc_seed.reap_num == nil and _otc_seed.total_seed >= _self_land_num) or
               (_otc_seed.total_seed >= (_otc_seed.reap_num + _self_land_num)),"剩余的种子没有那么多了，请等会儿再来！")
    -- 放入背包
    CPlayerItems.create_item_to_package(seed_id, _self_land_num)
end

-- 收获义种
local function _reap_seed() end
